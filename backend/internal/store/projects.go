...
func (s *Store) ListProjects() ([]Project, error) {
	rows, err := s.db.Query(`SELECT id, name, color, sort_order FROM project ORDER BY sort_order, id`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		var p Project
		if err := rows.Scan(&p.ID, &p.Name, &p.Color, &p.SortOrder); err != nil {
			return nil, err
		}
		out = append(out, p)
	}
...
func (s *Store) CreateProject(p Project) (Project, error) {
	...
	res, err := s.db.Exec(
		`INSERT INTO project (name, color, sort_order) VALUES (?, ?, ?)`,
		p.Name, p.Color, p.SortOrder,
	)
	if err != nil {
		return Project{}, err
	}
...
func (s *Store) GetProject(id int64) (Project, error) {
	var p Project
	err := s.db.QueryRow(
		`SELECT id, name, color, sort_order FROM project WHERE id = ?`, id,
	).Scan(&p.ID, &p.Name, &p.Color, &p.SortOrder)
